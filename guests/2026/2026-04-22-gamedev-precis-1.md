...
Добавим помимо анимации `idle` анимацию `run`. Для этого сначала нажмите в открытом `SpriteFrames` кнопку слева-сверху с зелёным знаком `+`. Затем переименуем новую анимацию на `run` (1) и выберем спрайт для этой анимации (2). 
...
func _physics_process(delta: float) -> void:
    ...
    if move_dir:
        ...
        $AnimatedSprite2D.play("idle") # Если игрок стоит, то проигрываем анимацию стояния на месте
    
    if move_dir.x: # Если мы двигаемся по горизонтали, то:
